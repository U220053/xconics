...
export default function ProductMasterListView() {
  ...
  const deleteSelectedGroups = async (selectedIds) => {
    const deletePromises = selectedIds.map((id) => deleteproduct(id));
        ...
        <CustomBreadcrumbs
          ...
          action={
            <Button
              ...
              href={paths.dashboard.product.masternew}
              variant="contained"
              ...
              <Tab
                ...
                icon={
                  ...
                  >
                    ...
                    {tab.value === 'active' &&
                      tableData.filter((product) => product.status === 1).length}

                    {tab.value === 'inactive' &&
                      tableData.filter((product) => product.status === 0).length}
                  </Label>
                }
              />
            ))}
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (status !== 'all') {
    if (status === 'active') {
      inputData = inputData.filter((product) => product.status === 1);
    } else {
      inputData = inputData.filter((product) => product.status === 0);
    }
  }
