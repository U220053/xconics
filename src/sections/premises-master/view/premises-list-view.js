...
import PremisesTableRow from '../premises-table-row';
import PremisesTableToolbar from '../premises-table-toolbar';
import PremisesTableFiltersResult from '../premises-table-filters-result';
import ExportToExcelPremises from '../premises-export-excel';
...
const TABLE_HEAD = [
  { id: 'premises_name', label: 'Premises_Name' },
  { id: 'premises_type', label: 'Premises_Type', width: 180 },
  { id: 'lat', label: 'Lat', width: 220 },
  { id: 'lang', label: 'Lang', width: 180 },
  { id: 'svg_tag', label: 'svg_tag', width: 180 },
  { id: 'activation_date', label: 'Activation_Date', width: 180 },
...
export default function PremisesListView() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const response = await axios.get('api/location/premises');
  ...
  async function deletegroup(id) {
    await axios.post(`api/location/premises/delete/${id}`);
  }
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.location.premisesedit(id));
    },
    [router]
  );
  ...
  const exportToPDF = () => {
    ...
    doc.save('Premises Master list.pdf');
  };
  ...
  return (
    <>
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          heading="Premises Master List"
          links={[
            { name: 'Dashboard', href: paths.dashboard.root },
            { name: 'Premises', href: paths.dashboard.location.root },
            // { name: 'Permission' },
          ]}
          action={
            <Button
              component={RouterLink}
              href={paths.dashboard.location.premisesnew}
              variant="contained"
        ...
        />
        <div>
          <ExportToExcelPremises data={dataFiltered} filename="Premises_Data" />
          <Button
            onClick={exportToPDF}
            style={divStyle}
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    inputData = inputData.filter(
      (user) => user.premises_name.toLowerCase().indexOf(name.toLowerCase()) !== -1
    );
  }
