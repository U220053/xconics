...
import TableContainer from '@mui/material/TableContainer';
 import ExportToExcellocation from '../location-export-excel';
// routes
...
const TABLE_HEAD = [
  { id: 'location_name', label: 'Location Name', width: 180 },
  { id: 'location_type', label: 'Location Type', width: 180 },
...
export default function LocationListView() {
  ...
  async function deletelocation(id) {
    await axios.post(`api/location/delete/${id}`);
  }
  const deleteSelectedGroups = async (selectedIds) => {
    const deletePromises = selectedIds.map((id) => deletelocation(id));
       ...
       <Container maxWidth={settings.themeStretch ? false : 'lg'}>
         <CustomBreadcrumbs
           ...
           links={[
             ...
             { name: 'location', href: paths.dashboard.location.root },
             { name: 'List' },
           ]}
           action={
             <Button
         ...
         />
  <div>
           <ExportToExcellocation data={dataFiltered} filename="Location_Data" />
           <Button
             onClick={exportToPDF}
             style={divStyle}
               ...
               <Tab
                 ...
                 icon={
                   ...
                   >
                     ...
                     {tab.value === 'active' &&
                       tableData.filter((location) => location.status === 1).length}

                     {tab.value === 'inactive' &&
                       tableData.filter((location) => location.status === 0).length}
                   </Label>
                 }
               />
             ))}
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (status !== 'all') {
    if (status === 'active') {
      inputData = inputData.filter((location) => location.status === 1);
    } else {
      inputData = inputData.filter((location) => location.status === 0);
    }
  }
